...
def all_expenses_of_user():
    ...
    fronted_expenses = Expense.query.options(joinedload(Expense.user)).options(
        joinedload(Expense.recipient)).filter(Expense.user_id == user_id).all()

    owe_others_expenses = Expense.query.options(joinedload(Expense.user)).options(
        joinedload(Expense.recipient)).filter(Expense.recipient_id == user_id).all()
    ...
    accounts_receivable = [fronted_expenses_to_dict(
        expense) for expense in fronted_expenses]
    accounts_payable = [owe_others_expenses_to_dict(
        expense) for expense in owe_others_expenses]
...
def get_one_expense(expense_id):
    ...
    expense = Expense.query.options(joinedload(Expense.recipient)).options(
        joinedload(Expense.user)).filter(Expense.id == expense_id).first()
    print(expense, '------------------------')
    if (not expense):
...
def create_expense():
    ...
    if form.validate_on_submit():
        ...
        new_expense = Expense(description=description, user_id=user_id,
                              group_id=group_id, recipient_id=recipient_id, amount=amount, note=note)
...
def update_expense(expense_id):
    ...
    id_of_user = current_user.id
    # if not found, expense will be None
